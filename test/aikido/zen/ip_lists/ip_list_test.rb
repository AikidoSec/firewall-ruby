...
require "test_helper"
require "aikido/zen/ip_lists/ip_list"

class Aikido::Zen::IPLists::IPListTest < ActiveSupport::TestCase
  def reader_with_ranges(family:, ranges:)
    io = StringIO.new(+"", "r+")
    io.set_encoding(Encoding::BINARY)
    Aikido::Zen::IPLists::IPList::Writer.new(family: family).write(io, ranges)
    Aikido::Zen::IPLists::IPList::Reader.new(family: family, io: io)
  end

  def ip_list(ipv4_ranges:, ipv6_ranges:, data: nil)
    Aikido::Zen::IPLists::IPList.new(
  ...
  test "#data returns the value it was given" do
    list = ip_list(ipv4_ranges: [], ipv6_ranges: [], data: "arbitrary bytes")

    assert_equal "arbitrary bytes", list.data
  end

  test "#data defaults to nil" do
    list = ip_list(ipv4_ranges: [], ipv6_ranges: [])

    assert_nil list.data
  end
  ...
  test "#include? does not match ipv4 addresses against ipv6 ranges, or vice versa" do
