...
class Aikido::Zen::Sinks::HTTPTest < ActiveSupport::TestCase
  class SSRFDetectionTest < ActiveSupport::TestCase
    ...
    test "allows normal requests" do
      refute_attack do
        response = HTTP.get("https://example.com/safe")
    ...
    test "prevents requests to hosts that come from user input" do
      ...
      assert_attack Aikido::Zen::Attacks::SSRFAttack do
        HTTP.get("https://example.com/safe")
      end
    ...
    test "raises a useful error message" do
      ...
      assert_equal \
        "SSRF: Request to user-supplied hostname «example.com» detected in http.request (GET https://example.com/safe).",
        error.message
    end
