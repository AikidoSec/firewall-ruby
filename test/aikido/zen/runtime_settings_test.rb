...
class Aikido::Zen::RuntimeSettingsTest < ActiveSupport::TestCase
  ...
  test "#update_from_json from a JSON response without the block key" do
    assert @settings.update_from_json({
      "success" => true,
      "serviceId" => 1234,
      "configUpdatedAt" => 1717171717,
  ...
  test "#update_from_json should return true or false indicating whether the settings were updated" do
    payload = {
      "success" => true,
      "serviceId" => 1234,
      "configUpdatedAt" => 1717171717,
  ...
  test "#update_from_json populates excluded_user_ids_from_rate_limiting" do
  ...
  test "#update_from_json sets enabled_features" do
    @settings.update_from_json({"enabledFeatures" => ["realtime_updates"]})
    assert_includes @settings.enabled_features, "realtime_updates"

    @settings.update_from_json({"enabledFeatures" => []})
  ...
  test "#update_from_json replaces the excluded user ids set" do
    @settings.update_from_json({
      "excludedUserIdsFromRateLimiting" => ["user1", "user2"]
    })
    @settings.update_from_json({
  ...
  test "#update_from_json with an empty array clears the excluded user ids" do
    @settings.update_from_json({
