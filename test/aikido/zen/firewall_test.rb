...
class Aikido::Zen::FirewallTest < ActiveSupport::TestCase
  ...
  test "#update_from_json from an empty JSON response" do
    @firewall.update_from_json({}, ip_lists_directory: @ip_lists_directory)
  ...
  test "#update_from_json from a JSON response" do
    @firewall.update_from_json(
      ...
      ip_lists_directory: @ip_lists_directory
    )

    assert_kind_of Aikido::Zen::IPLists::Data::Reader, @firewall.blocked_ip_lists
    refute @firewall.blocked_ip_lists.empty?

    blocked_matches = @firewall.matching_blocked_lists("1.4.9.1")
    ...
    assert_kind_of Aikido::Zen::IPLists::Data::Reader, @firewall.allowed_ip_lists
    refute @firewall.allowed_ip_lists.empty?
    assert_equal ["key2"], @firewall.allowed_ip_lists.matching_lists("2.63.192.1").map(&:key)
    assert_equal ["key3"], @firewall.allowed_ip_lists.matching_lists("5.8.8.1").map(&:key)
    refute @firewall.allowed_ip_lists.include?("8.8.8.8")
