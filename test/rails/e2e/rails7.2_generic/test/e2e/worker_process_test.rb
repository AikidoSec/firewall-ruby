...
class WorkerProcessTest < ActiveSupport::TestCase
  ...
  module GenericTests
    ...
    test "worker processes pick up updated settings" do
  ...
  class IndirectModeTest < ActiveSupport::TestCase
    ...
    test "worker process enforces rate limits shared across workers" do
      configure_mock(
        "endpoints" => [{
          "method" => "GET",
  ...
  class DirectModeTest < ActiveSupport::TestCase
    ...
    test "worker collector events are reported via each worker's own heartbeat" do
      ...
      poll_responses_by_pid(WORKER_COUNT, timeout: 10) { rails_get("/test/worker_process") }

      # Wait for settings fetch to finish.
      sleep 2
      ...
      heartbeats = poll_until(timeout: 15) do
        heartbeats = received_events(type: "heartbeat")[baseline..].map { |h| h.dig("stats", "requests", "attacksDetected") }
        heartbeats if heartbeats.sum { |a| a["total"].to_i } >= attack_count
      end

      assert_operator heartbeats.sum { |a| a["total"].to_i }, :>=, attack_count
      assert_operator heartbeats.sum { |a| a["blocked"].to_i }, :>=, attack_count
    end

    test "worker process enforces rate limits locally per worker" do
      configure_mock(
        "endpoints" => [{
          "method" => "GET",
          "route" => "/test/rate_limit(.:format)",
      ...
      responses_by_pid = Hash.new { |h, k| h[k] = [] }

      poll_until(timeout: 15, interval: 0) do
        response = rails_get("/test/rate_limit")

        responses_by_pid[response["X-Worker-Pid"]] << response.code

        responses_by_pid.size >= WORKER_COUNT && responses_by_pid.values.all? { |codes| codes.length >= 3 }
      end

      responses_by_pid.each do |pid, codes|
        assert_equal ["200", "200", "429"], codes.first(3),
          "expected worker #{pid} to enforce its own 2-request limit independently of other workers"
      end
    end
  end
end
